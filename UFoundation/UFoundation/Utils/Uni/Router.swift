...
open class Router: NSObject {
    ...
    fileprivate func open(_ key: String, params: RouterParam?, options: RouterOptions?, animated: Bool, completion: ((_ opened: UIViewController?) -> Void)?, unused: Bool) {
        ...
        if nav.presentedViewController != nil {
            print("dismiss the vc that has been presented:\(nav.presentedViewController ?? UIViewController())")
    ...
    open func openExternal(_ url: String, type: RouterType = .safari) {
        guard let routerURL = type.router(url) else {
            return
        }
        
        UIApplication.shared.open(routerURL, options: [:], completionHandler: nil)
    }
